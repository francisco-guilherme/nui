...
function Toolbar({
  ...
  return (
    <BaseToolbar.Root
      className={cn(
        "flex items-center gap-1 rounded-md bg-popover p-1 outline outline-border",
        className
      )}
      {...props}
    />
...
function ToolbarLink({
  ...
  return (
    <BaseToolbar.Link
      className={cn(
        "inline-flex h-9 shrink-0 items-center gap-2 rounded-md px-4 text-muted-foreground text-sm no-underline transition-colors hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/50 [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className
      )}
      {...props}
    />
